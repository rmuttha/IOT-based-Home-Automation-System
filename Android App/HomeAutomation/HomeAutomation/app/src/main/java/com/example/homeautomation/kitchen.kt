 ...
 *
 * @brief   File for the kitchen fragment
 *
 ******************************************************************************/
...
class kitchen : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // To display if smoke  is detected
        smokeTextView = binding.smoke

        // Click listeners to return to homepage
        binding.homepageButton.setOnClickListener {
            findNavController().navigate(R.id.action_kitchen_to_homepage)
        }

        // Click listeners to turn on light
        binding.lightOnButton.setOnClickListener {
            publishMessage(TURNON_MESSAGE, ESP_KR_LITEON_TOPIC, enableOnBtn = false, enableOffBtn = true)
            publishTopic(ESP_KR_LITEON_TOPIC)
        }

        // Click listeners to turn off light
        binding.lightOffButton.setOnClickListener {
            publishMessage(TURNOFF_MESSAGE, ESP_KR_LITEOFF_TOPIC, enableOnBtn = true, enableOffBtn = false)
            publishTopic(ESP_KR_LITEOFF_TOPIC)
        }

        // Click listeners to open the door
        binding.doorOpenButton.setOnClickListener {
            publishMessage(ESP_KR_DOOROPEN, ESP_KR_DOOROPEN_TOPIC, enableOnBtn = false, enableOffBtn = true)
            publishTopic(ESP_KR_DOOROPEN_TOPIC)
        }

        // Click listeners to close door
        binding.doorCloseButton.setOnClickListener {
            publishMessage(ESP_KR_DOORCLOSE, ESP_KR_DOORCLOSE_TOPIC, enableOnBtn = true, enableOffBtn = false)
            publishTopic(ESP_KR_DOORCLOSE_TOPIC)
        }
            ...
            mqttClient.connect(
                ...
                object : IMqttActionListener {
                    ...
                    override fun onSuccess(asyncActionToken: IMqttToken?) {
                        ...
                        subscribeToTopic(ESP_KR_LITE_TOPIC)
                        subscribeToTopic(ESP_KR_DOOR_TOPIC)
                        subscribeToTopic(ESP_SMOKE_TOPIC)
                    }
                ...
                object : MqttCallback {
                    ...
                    override fun messageArrived(topic: String?, message: MqttMessage?) {
                        ...
                        Log.d(TAG, msg)
                        if (message.toString() == SMOKE_DETECTED_MESSAGE) {
                            smokeTextView.text =
                                message?.payload?.toString(Charsets.UTF_8)
                            binding.smokeImage.setImageResource(R.drawable.aftersmoke)
                        }
                    }
            ...
            mqttClient.publish(
                ...
                object : IMqttActionListener {
                    ...
                    override fun onSuccess(asyncActionToken: IMqttToken?) {
                        ...
                        if((selectedTopic == ESP_KR_LITEON_TOPIC) || (selectedTopic == ESP_KR_LITEOFF_TOPIC)){
                            binding.lightOnButton.isEnabled = enableOnBtn
                            binding.lightOffButton.isEnabled = enableOffBtn
                        }else if((selectedTopic == ESP_KR_DOOROPEN_TOPIC) || (selectedTopic == ESP_KR_DOORCLOSE_TOPIC)){
