 ...
 *
 * @brief   File for the living room fragment
 *
 ******************************************************************************/
...
class livingroom : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding = FragmentLivingroomBinding.inflate(inflater,container,false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.homepageButton.setOnClickListener {
            findNavController().navigate(R.id.action_livingroom_to_homepage)
        }

        // Click listeners to turn on light
        binding.lightOnButton.setOnClickListener {
            publishMessage(TURNON_MESSAGE, ESP_LR_LITEON_TOPIC, enableOnBtn = false, enableOffBtn = true)
            publishTopic(ESP_LR_LITEON_TOPIC)
        }

        // Click listeners to turn off light
        binding.lightOffButton.setOnClickListener {
            publishMessage(TURNOFF_MESSAGE, ESP_LR_LITEOFF_TOPIC, enableOnBtn = true, enableOffBtn = false)
            publishTopic(ESP_LR_LITEOFF_TOPIC)
        }

        // Click listeners to turn on fan
        binding.fanOnButton.setOnClickListener {
            publishMessage(TURNON_MESSAGE, ESP_LR_FANON_TOPIC, enableOnBtn = false, enableOffBtn = true)
            publishTopic(ESP_LR_FANON_TOPIC)
        }

        // To turn off fan
        binding.fanOffButton.setOnClickListener {
            publishMessage(TURNOFF_MESSAGE, ESP_LR_FANOFF_TOPIC, enableOnBtn = false, enableOffBtn = true)
            publishTopic(ESP_LR_FANOFF_TOPIC)
        }
        ...
        binding.fanOnButton.isEnabled = false
        binding.fanOffButton.isEnabled = false
            ...
            mqttClient.connect(
                ...
                object : IMqttActionListener {
                    ...
                    override fun onSuccess(asyncActionToken: IMqttToken?) {
                        ...
                        binding.fanOnButton.isEnabled = true
                        binding.fanOffButton.isEnabled = true

                        //subscribe to the status topic
                        subscribeToTopic(STS_TOPIC)
                        subscribeToTopic(ESP_LR_LITE_TOPIC)
                        subscribeToTopic(ESP_TEMP_TOPIC)
                        subscribeToTopic(ESP_LR_FAN_TOPIC)
                    }
            ...
            mqttClient.publish(
                ...
                object : IMqttActionListener {
                    ...
                    override fun onSuccess(asyncActionToken: IMqttToken?) {
                        ...
                        if((selectedTopic == ESP_LR_LITEON_TOPIC) || (selectedTopic == ESP_LR_LITEOFF_TOPIC)){
                            binding.lightOnButton.isEnabled = enableOnBtn
                            binding.lightOffButton.isEnabled = enableOffBtn
                        }else if((selectedTopic == ESP_LR_FANON_TOPIC) || (selectedTopic == ESP_LR_FANOFF_TOPIC)){
                            binding.fanOnButton.isEnabled = enableOnBtn
                            binding.fanOffButton.isEnabled = enableOffBtn
                        }
                        else
                        {
