 ...
 *
 * @brief   File for the bedroom fragment
 *
 ******************************************************************************/
...
class bedroom : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.homepageButton.setOnClickListener {
            findNavController().navigate(R.id.action_bedroom_to_homepage)
        }

        // Click listeners to turn on light
        binding.lightOnButton.setOnClickListener {
            publishMessage(TURNON_MESSAGE, ESP_BR_LITEON_TOPIC, enableOnBtn = false, enableOffBtn = true)
            publishTopic(ESP_BR_LITEON_TOPIC)
        }

        // Click listeners to turn off light
        binding.lightOffButton.setOnClickListener {
            publishMessage(TURNOFF_MESSAGE, ESP_BR_LITEOFF_TOPIC, enableOnBtn = true, enableOffBtn = false)
            publishTopic(ESP_BR_LITEOFF_TOPIC)
        }

        // Click listeners to open door
        binding.doorOpenButton.setOnClickListener {
            publishMessage(ESP_BR_DOOROPEN, ESP_BR_DOOROPEN_TOPIC, enableOnBtn = false, enableOffBtn = true)
            publishTopic(ESP_BR_DOOROPEN_TOPIC)
        }

        // Click listeners to close door
        binding.doorCloseButton.setOnClickListener {
            publishMessage(ESP_BR_DOORCLOSE, ESP_BR_DOORCLOSE_TOPIC, enableOnBtn = true, enableOffBtn = false)
            publishTopic(ESP_BR_DOORCLOSE_TOPIC)
        }

        // Click listeners to open curtain
        binding.curtainOpenButton.setOnClickListener {
            publishMessage(ESP_BR_CURTAINOPEN, ESP_BR_CURTAINOPEN_TOPIC, enableOnBtn = false, enableOffBtn = true)
            publishTopic(ESP_BR_CURTAINOPEN_TOPIC)
        }

        // Click listeners to close curtain
        binding.curtainCloseButton.setOnClickListener {
            publishMessage(ESP_BR_CURTAINCLOSE, ESP_BR_CURTAINCLOSE_TOPIC, enableOnBtn = true, enableOffBtn = false)
            publishTopic(ESP_BR_CURTAINCLOSE_TOPIC)
        }
        ...
        binding.curtainOpenButton.isEnabled = false
        binding.curtainCloseButton.isEnabled = false
            ...
            mqttClient.connect(
                ...
                object : IMqttActionListener {
                    ...
                    override fun onSuccess(asyncActionToken: IMqttToken?) {
                        ...
                        binding.curtainOpenButton.isEnabled = true
                        binding.curtainCloseButton.isEnabled = true

                        //subscribe to the status topic
                        subscribeToTopic(STS_TOPIC)
                        subscribeToTopic(ESP_BR_LITE_TOPIC)
                        subscribeToTopic(ESP_BR_CURTAIN_TOPIC)
                        subscribeToTopic(ESP_BR_DOOR_TOPIC)
                    }
    ...
    // Function to publish the topics to MQTT
            ...
            mqttClient.publish(
                ...
                object : IMqttActionListener {
                    ...
                    override fun onSuccess(asyncActionToken: IMqttToken?) {
                        ...
                        if((selectedTopic == ESP_BR_LITEON_TOPIC) || (selectedTopic == ESP_BR_LITEOFF_TOPIC)){
                            binding.lightOnButton.isEnabled = enableOnBtn
                            binding.lightOffButton.isEnabled = enableOffBtn
                        }else if((selectedTopic == ESP_BR_DOOROPEN_TOPIC) || (selectedTopic == ESP_BR_DOORCLOSE_TOPIC)){
                            binding.doorOpenButton.isEnabled = enableOnBtn
                            binding.doorCloseButton.isEnabled = enableOffBtn
                        }else if((selectedTopic == ESP_BR_CURTAINOPEN_TOPIC) || (selectedTopic == ESP_BR_CURTAINCLOSE_TOPIC)){
                            binding.curtainOpenButton.isEnabled = enableOnBtn
                            binding.curtainCloseButton.isEnabled = enableOffBtn
                        }
                        else
                        {
