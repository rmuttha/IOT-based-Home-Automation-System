 ...
 *
 * @brief   File for the garage fragment
 *
 ******************************************************************************/
...
class garage : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.homepageButton.setOnClickListener {
            findNavController().navigate(R.id.action_garage_to_homepage)
        }

        // Click listeners to turn on light
        binding.lightOnButton.setOnClickListener {
            publishMessage(TURNON_MESSAGE, ESP_GR_LITEON_TOPIC, enableOnBtn = false, enableOffBtn = true)
            publishTopic(ESP_GR_LITEON_TOPIC)
        }

        // Click listeners to turn off light
        binding.lightOffButton.setOnClickListener {
            publishMessage(TURNOFF_MESSAGE, ESP_GR_LITEOFF_TOPIC, enableOnBtn = true, enableOffBtn = false)
            publishTopic(ESP_GR_LITEOFF_TOPIC)
        }

        // Click listeners to open door
        binding.doorOpenButton.setOnClickListener {
            publishMessage(ESP_GR_DOOROPEN, ESP_GR_DOOROPEN_TOPIC, enableOnBtn = false, enableOffBtn = true)
            publishTopic(ESP_GR_DOOROPEN_TOPIC)
        }

        // Click listeners to close door
        binding.doorCloseButton.setOnClickListener {
            publishMessage(ESP_GR_DOORCLOSE, ESP_GR_DOORCLOSE_TOPIC, enableOnBtn = true, enableOffBtn = false)
            publishTopic(ESP_GR_DOORCLOSE_TOPIC)
        }
            ...
            mqttClient.connect(
                ...
                object : IMqttActionListener {
                    ...
                    override fun onSuccess(asyncActionToken: IMqttToken?) {
                        ...
                        subscribeToTopic(ESP_GR_LITE_TOPIC)
                        subscribeToTopic(ESP_GR_DOOR_TOPIC)
                    }
            ...
            mqttClient.publish(
                ...
                object : IMqttActionListener {
                    ...
                    override fun onSuccess(asyncActionToken: IMqttToken?) {
                        ...
                        if((selectedTopic == ESP_GR_LITEON_TOPIC) || (selectedTopic == ESP_GR_LITEOFF_TOPIC)){
                            binding.lightOnButton.isEnabled = enableOnBtn
                            binding.lightOffButton.isEnabled = enableOffBtn
                        }else if((selectedTopic == ESP_GR_DOOROPEN_TOPIC) || (selectedTopic == ESP_GR_DOORCLOSE_TOPIC)){
